...
describe('endpoint - /photos/upload', () => {
	...
	test('POST jpeg file - should respond with 201', async() => {
		const res: request.Response = await request(app).post('/api/v1/photos/upload').attach('image', path.join(__dirname, '../', 'testfiles/', 'image.jpg'));
		expect(res.body.message).toEqual('Photo Uploaded');
		expect(res.status).toEqual(201);
	});

	test('POST txt file - should respond with 404', async() => {
		const res: request.Response = await request(app).post('/api/v1/photos/upload').attach('image', path.join(__dirname, '../', 'testfiles/', 'note.txt'));
		expect(res.status).toEqual(404);
	})
});
